...
<div>
    @if ($paginator->hasPages())
        <nav role="navigation" aria-label="Pagination Navigation lol" class="flex items-center justify-between">
            <div class="flex gap-10 flex-1 sm:hidden">
                ...
                <span>
                    @if ($paginator->hasMorePages())
                        <button type="button" wire:click="nextPage('{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" wire:loading.attr="disabled" dusk="nextPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}.before" class="inline-flex items-center p-1 text-gray-700 hover:text-gray-500 focus:outline-none focus:ring ring-blue-300 transition ease-in-out duration-150">
                            <x-icons.arrow-right/>
                        </button>
                    @else
            ...
            <div class="hidden gap-3 sm:flex-1 sm:flex sm:items-center sm:justify-between">
                ...
                <div>
                    <span class="relative z-0 inline-flex rtl:flex-row-reverse items-center ">
                        <span>
                            {{-- Previous Page Link --}}
                            @if ($paginator->onFirstPage())
                                <span aria-disabled="true" aria-label="{{ __('pagination.previous') }}">
                                    <span class="relative inline-flex items-center px-2 py-2 text-sm font-medium text-black  cursor-default leading-5 " aria-hidden="true">
                        ...
                        @foreach ($elements as $element)
                            ...
                            @if (is_string($element))
                                <span aria-disabled="true">
                                    <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-zinc-800 cursor-default leading-5 ">{{ $element }}</span>
                                </span>
                            @endif
                                    ...
                                    <span wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $page }}">
                                        @if ($page == $paginator->currentPage())
                                            <span aria-current="page">
                                                <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-medium text-amber-300 underline cursor-default leading-5 ">{{ $page }}</span>
                                            </span>
                                        @else
                        ...
                        <span>
                            {{-- Next Page Link --}}
                            @if ($paginator->hasMorePages())
                                <button type="button" wire:click="nextPage('{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" dusk="nextPage{{ $paginator->getPageName() == 'page' ? '' : '.' . $paginator->getPageName() }}.after" class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-zinc-700 bg-white  rounded-r-md leading-5 hover:text-gray-400 focus:z-10 transition ease-in-out duration-150 " aria-label="{{ __('pagination.next') }}">
                                    <x-icons.arrow-right/>
                                </button>
                            @else
                                <span aria-disabled="true" aria-label="{{ __('pagination.next') }}">
                                    <span class="relative inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-zinc-800  cursor-default rounded-r-md leading-5 " aria-hidden="true">
                                        <x-icons.arrow-right/>
                                    </span>
                                </span>
                            @endif
