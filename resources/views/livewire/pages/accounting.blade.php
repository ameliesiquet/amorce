    ...
    <section class="flex flex-row gap-10">
        @foreach($funds as $fund)
            <div class="flex flex-col gap-4">
                ...
                <livewire:fund-transactions :fund="$fund" />
            </div>
        @endforeach
    </section>
    <section class="flex flex-wrap gap-10 justify-between items-center">
        <div class="flex flex-wrap gap-9 items-center self-stretch my-auto text-base">
            <!-- Add Button  -->
            <x-yellow-button>
                <x-icons.add/>
                Ajouter un fond
            ...
            <!-- Search Form-->
            <form
                class="flex gap-1.5 items-center self-stretch p-3 my-auto rounded-xl border border-solid border-neutral-400 text-zinc-500">
                <label for="searchInput" class="sr-only">Rechercher un font</label>
                <x-icons.search/>
                <input type="search" id="searchInput" placeholder="Rechercher un font"
